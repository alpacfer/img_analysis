...
def show_in_moved_window(win_name, img, x, y):
    ...
    # Change the window size to specified size
...
def process_gray_image(img):
    ...
    # proc_img = median(proc_img, footprint=kernel) # type: ignore

    # Filter the image with a gaussian filter
    proc_img = gaussian(proc_img, sigma=2) # type: ignore

    # Apply Prewitt filter
    proc_img = prewitt(proc_img) # type: ignore

    # Threshold the image with Otsu's method
    thresh = threshold_otsu(proc_img) # type: ignore
    proc_img = proc_img > thresh
...
def capture_from_camera_and_show_images():
    ...
    while not stop:
        ...
        cv2.putText(new_frame, str_out, (10, 30), font, 1, (255, 255, 255), 2, cv2.LINE_AA)

        # Display the resulting frame
        show_in_moved_window('Input', new_frame, 0, 10)
        show_in_moved_window('Processed image', proc_img, 500, 10)
